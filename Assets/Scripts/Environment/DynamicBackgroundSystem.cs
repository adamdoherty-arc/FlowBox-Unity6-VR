...
namespace VRBoxingGame.Environment
{
    ...
    // Component for objects that should react to music
    public class ReactiveEnvironmentObject : MonoBehaviour
    {
        ...
        public float reactivityMultiplier = 1f;
        public int frequencyBand = 0; // Which frequency band to react to (0-7)
        ...
        private void Update()
        {
            if (DynamicBackgroundSystem.Instance == null) return;
            
            // Get frequency data from audio manager
            float[] frequencyBands = AdvancedAudioManager.Instance?.GetFrequencyBands();
            if (frequencyBands == null || frequencyBand >= frequencyBands.Length) return;
            
            float energy = frequencyBands[frequencyBand] * reactivityMultiplier;
            
            // Apply scale reaction
            if (reactToScale)
            {
                float scaleMultiplier = 1f + (energy * 0.5f);
                transform.localScale = Vector3.Lerp(transform.localScale, originalScale * scaleMultiplier, Time.deltaTime * 5f);
            }
            
            // Apply rotation reaction
            if (reactToRotation)
            {
                float rotationSpeed = energy * 100f;
                transform.Rotate(0, rotationSpeed * Time.deltaTime, 0);
            }
            
            // Apply color reaction
            if (reactToColor && objectRenderer != null)
            {
                Color targetColor = Color.Lerp(originalColor, Color.HSVToRGB(energy, 1f, 1f), energy);
